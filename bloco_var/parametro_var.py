...
import valor_variavel
import parametro_vetor
        ...
        if(len(self.token)>0):  
            match self.list[0]:
                case ';':
                    ...
                    if(len(self.list)> 0):
                        if self.list[0] == 'true' or self.list[0] == 'false' or self.token[0] == "IDE" or self.token[0] == "CAC" or self.token == "NRO" or self.list[0] == 'int' or self.list[0] == 'boolean' or self.list[0] == 'string' or self.list[0] == 'real':
                            ...
                            iniciar_automato = automato_bloco_var.bloco_var(self.list, self.n, self.erro, self.token)
                            iniciar_automato.E2()
                    else:
                        iniciar_automato = automato_bloco_var.bloco_var(self.list, self.n, self.erro, self.token)
                        iniciar_automato.E2()
                ...
                case '=':
                    ...
                    self.list.pop(0)
                    self.token.pop(0)
                    iniciar_automato = valor_variavel.valor_variavel(self.list,self.n, self.erro,self.token)
                    iniciar_automato.E0()
                case '[':
                    self.n.pop(0)
                    self.list.pop(0)
                    self.token.pop(0)
                    iniciar_automato = parametro_vetor.parametro_vetor(self.list, self.n, self.erro, self.token)
                    iniciar_automato.E0()
                case _:
