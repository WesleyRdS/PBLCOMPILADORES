        ...
        if(len(self.token)>0):  
            match self.list[0]:
                ...
                case '=':
                    ...
                    self.list.pop(0)
                    self.token.pop(0)
                    iniciar_automato = constante.valor_variavelC.valor_variavelC(self.list,self.n, self.erro,self.token)
                    iniciar_automato.E0()
                case '[':
                    self.n.pop(0)
                    self.list.pop(0)
                    self.token.pop(0)
                    iniciar_automato = constante.parametro_vetorC.parametro_vetorC(self.list, self.n, self.erro, self.token)
                    iniciar_automato.E0()
                case _:
                ...
                case ';':
                    ...
                    if(len(self.list)> 0):
                        if self.list[0] == 'true' or self.list[0] == 'false' or self.token[0] == "IDE" or self.token == "NRO" or self.list[0] == 'int' or self.list[0] == 'boolean' or self.list[0] == 'string' or self.list[0] == 'real' or self.token[0] == "CAC":
                            ...
                            iniciar_automato = constante.automato_bloco_const.bloco_const(self.list, self.n, self.erro, self.token)
                            iniciar_automato.E2()
                    else:
                        iniciar_automato = constante.automato_bloco_const.bloco_const(self.list, self.n, self.erro, self.token)
                        iniciar_automato.E2()
